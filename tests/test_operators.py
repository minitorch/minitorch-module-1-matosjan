...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert sigmoid(a) < sigmoid(a + 0.1) + 1e-6
...
def test_other(x: float, y: float, z: float) -> None:
    ...
    if pytest.approx(x, 1e-6) != 0:
        assert pytest.approx(mul(x, inv(x)), 1e-6) == 1
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    """
    assert is_close(add(sum(ls1, 0), sum(ls2, 0)), sum(addLists(ls1, ls2)))
